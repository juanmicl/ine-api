...
class AsyncClient:
    ...
    Espejo asincrono de :class:`~ine.client.Client`: misma API y mismos
    parámetros (todos *keyword-only*), sustituyendo :class:`httpx.Client` por
    :class:`httpx.AsyncClient`. Todos los endpoints son coroutines y deben
    esperarse con ``await``.

    Uso típico como gestor de contexto asincrono (cierra la conexión HTTP al
    salir)::
